"""
Fix remaining __ links - correct version.
Old links are RELATIVE to std dir: 化学成分__甲醇含量 (no std_slug prefix)
"""
import os
import re
...
# Step 1: Build old_basename -> new_basename mapping
# Old basename (relative to std dir): cat_slug__subcat_slug__grade_slug__expr_slug
old_to_new = {}  # (cat_dir, std_dirname, old_bn_relative) -> new_bn
...
for root, dirs, files in os.walk(base):
    ...
    for f in files:
        ...
        # Build old basename RELATIVE to std dir (no std_slug prefix)
        if ntype == 'indicator-category':
            ...
            old_bn = f'{cat_slug}__{sub_slug}__{grade_slug}__{expr_slug}'
        else:
            continue
...
# Show some examples
examples = list(old_to_new.items())[:5]
print('\nExamples:')
for (cd, sd, obn), nbn in examples:
    print(f'  ({cd}, {sd}, {obn}) -> {nbn}')
    ...
    for f in files:
        ...
        for link in links:
            ...
            if key in old_to_new:
                ...
                # Try without the display part
                if '|' in link:
                    bare_path = path
                    bare_key = (link_cat_dir, link_std_dirname, bare_path.split('/', 4)[4])
                    if bare_key in old_to_new:
                        new_bn = old_to_new[bare_key]
                        old_link = f'[[{link}]]'
                        display = link.split('|', 1)[1]
...
print(f'\nFixed {fixed} links out of {total}')
