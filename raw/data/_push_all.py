"""Push all files from local disk to GitHub via API"""
...
for root, dirs, files in os.walk(BASE):
    for f in files:
        ...
        # Skip .obsidian, .git, raw/data/_push*, __pycache__
        if rel.startswith('.obsidian/') or rel.startswith('.git/') or rel.startswith('raw/data/_push') or '__pycache__' in rel:
            continue
        if rel.endswith('.pyc'):
            continue
...
for i, rel_path in enumerate(files_to_push):
    ...
    url = f'{API}/{urllib.parse.quote(rel_path, safe="")}'
